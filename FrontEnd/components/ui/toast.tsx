  ...
  'group pointer-events-auto relative flex w-full items-center justify-between space-x-4 overflow-hidden rounded-lg border p-4 pr-8 shadow-lg transition-all data-[swipe=cancel]:translate-x-0 data-[swipe=end]:translate-x-[var(--radix-toast-swipe-end-x)] data-[swipe=move]:translate-x-[var(--radix-toast-swipe-move-x)] data-[swipe=move]:transition-none data-[state=open]:animate-in data-[state=closed]:animate-out data-[swipe=end]:animate-out data-[state=closed]:fade-out-80 data-[state=closed]:slide-out-to-right-full data-[state=open]:slide-in-from-top-full data-[state=open]:sm:slide-in-from-bottom-full',
  {
    variants: {
      variant: {
        ...
        info:
          'info group border-blue-200 bg-blue-50 text-blue-800 dark:border-blue-800 dark:bg-blue-950 dark:text-blue-200',
      },
    },
    defaultVariants: {
      variant: 'default',
...
const ToastIcon = ({ variant }: { variant?: "default" | "destructive" | "success" | "warning" | "info" | null }) => {
  ...
  switch (variant) {
    case 'success':
      return <CheckCircle {...iconProps} className="h-5 w-5 shrink-0 text-green-600 dark:text-green-400" />;
    case 'destructive':
      return <AlertCircle {...iconProps} className="h-5 w-5 shrink-0 text-red-600 dark:text-red-400" />;
    case 'warning':
      return <AlertTriangle {...iconProps} className="h-5 w-5 shrink-0 text-yellow-600 dark:text-yellow-400" />;
    case 'info':
      return <Info {...iconProps} className="h-5 w-5 shrink-0 text-blue-600 dark:text-blue-400" />;
    default:
      return null;
  }
};
